...
const queryPages = /* GraphQL */ `
  query($conferenceTitle: ConferenceTitle, $eventYear: EventYear, $title: String, $key: String) {
    result: pages(
      where: {
        conferenceEvent: { year: $eventYear, conferenceBrand: { title: $conferenceTitle } }
        titlePage_contains: $title
        key_contains: $key
      }
    ) {
      id
...
export const pages = Query({
  ...
  searchVars: { title: '', key: '' },
  viewId: '5f0d4bba1a384274b8259ed15cdcfea9',
  name: 'Pages',
});

export const texts = Query({
  query: queryTexts,
  vars: { conferenceTitle, eventYear },
  searchVars: { markdown: '', key: '' },
